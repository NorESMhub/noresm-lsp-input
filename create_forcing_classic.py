...
class SinglePointExtractor:
    """
    Utility class to create single-point CTSM forcing from global NetCDF files.
    """
    ...
    def _create_urban(self):
        ...
        out_path = self.output_dir / 'lnd' / \
            'clm2' / 'urbandata' / str(self.site_code)
        if not out_path.is_dir():
            self.make_dir(out_path)
        out_str = str(out_path)
    ...
    def _create_fire(self):
        ...
        out_path = self.output_dir / 'lnd' / \
            'clm2' / 'firedata' / str(self.site_code)
        if not out_path.is_dir():
            self.make_dir(out_path)
        out_str = str(out_path)
    ...
    def _create_atm_lightning(self):
        ...
        out_path = self.output_dir / 'atm' / \
            'datm7' / 'NASA_LIS' / str(self.site_code)
        if not out_path.is_dir():
            self.make_dir(out_path)
        out_str = str(out_path)
